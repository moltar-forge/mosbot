      ...
      for (let i = 0; i < pathParts.length; i++) {
        ...
        if (existingItem) {
          if (isLastPart) {
            ...
            showToast(
              `A ${existingItem.type === 'directory' ? 'folder' : 'file'} named "${part}" already exists at this location`,
              'error',
            );
            ...
            if (existingItem.type !== 'directory') {
              showToast(
                `Cannot create file: "${part}" already exists as a file in the path`,
                'error',
              );
              setIsSubmitting(false);
              return;
              ...
              <div>
                ...
                <div className="mt-2 space-y-1">
                  <p className="text-xs text-dark-500">
                    <span className="font-medium text-dark-400">Base location:</span>{' '}
                    {currentPath === '/' ? '/' : currentPath}
                  </p>
                  {isInsideSymlink && (
                    <p className="text-xs text-red-400">
                      <span className="font-medium">Warning:</span> Cannot create files inside
                      symlink directories
                    </p>
                  )}
                  {!isInsideSymlink && (
                    <p className="text-xs text-dark-500">
                      <span className="font-medium text-dark-400">Tip:</span> Use / to create nested
                      folders (e.g., docs/guides/setup.md)
                    </p>
                  )}
                </div>
              </div>
