...
export default function TaskCard({ task, onClick }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            {task.task_number && (
              <span className="text-xs font-mono text-dark-500">TASK-{task.task_number}</span>
            )}
          </div>
          <h3 className="text-sm font-semibold text-dark-100">{task.title}</h3>
        </div>
      ...
      {/* Summary (preferred) or description snippet */}
      ...
      {task.tags && task.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {task.tags.map((tag, index) => (
            <span key={index} className="px-2 py-0.5 text-xs bg-dark-800 text-dark-300 rounded">
              {tag}
            </span>
          ))}
        </div>
      ...
      {hasAIUsage(task) &&
        (() => {
          ...
          const totalOutput =
            (task.agent_tokens_output ?? 0) + (task.agent_tokens_output_cache ?? 0);
          ...
          if (totalTokens > 0) {
            ...
            const inputCacheStr =
              task.agent_tokens_input_cache != null ? task.agent_tokens_input_cache : '?';
            const outputStr = task.agent_tokens_output != null ? task.agent_tokens_output : '?';
            const outputCacheStr =
              task.agent_tokens_output_cache != null ? task.agent_tokens_output_cache : '?';
            tooltipLines.push(
              `Input: ${inputStr} (cache ${inputCacheStr}), Output: ${outputStr} (cache ${outputCacheStr})`,
            );
          }
          if (formattedCost) {
          ...
          return (
            <div className="text-xs text-primary-400 mb-3 font-mono" title={tooltip}>
              {displayText}
            </div>
          );
        })()}
