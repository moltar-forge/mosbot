...
describe('CreateFolderModal', () => {
  ...
  it('does not render when isOpen is false', () => {
    render(<CreateFolderModal isOpen={false} onClose={mockOnClose} currentPath="/" />);

    expect(screen.queryByText('Create New Folder')).not.toBeInTheDocument();
  });

  it('renders modal when open', () => {
    render(<CreateFolderModal isOpen={true} onClose={mockOnClose} currentPath="/" />);
  ...
  it('creates folder with correct path in subdirectory', async () => {
    ...
    render(<CreateFolderModal isOpen={true} onClose={mockOnClose} currentPath="/documents" />);
    ...
    expect(mockCreateDirectory).toHaveBeenCalledWith(
      expect.objectContaining({ path: '/documents/subfolder' }),
    );
  });
  ...
  it('shows error for invalid characters', async () => {
    ...
    expect(mockShowToast).toHaveBeenCalledWith('Folder name contains invalid characters', 'error');
  });
  ...
  it('shows error for path traversal attempts', async () => {
    ...
    expect(mockShowToast).toHaveBeenCalledWith('Folder name cannot contain / or ..', 'error');
  });
  ...
  it('trims whitespace from folder name', async () => {
    ...
    expect(mockCreateDirectory).toHaveBeenCalledWith(
      expect.objectContaining({ path: '/testfolder' }),
    );
  });
});
