...
export default function TaskModal({ isOpen, onClose, task = null }) {
  ...
  const loadComments = async (taskId, { force = false } = {}) => {
    ...
    try {
      ...
      showToast(error.response?.data?.error?.message || 'Failed to load comments', 'error');
    } finally {
  ...
  const loadSubtasks = async (taskId) => {
    ...
    try {
      ...
      showToast(error.response?.data?.error?.message || 'Failed to load subtasks', 'error');
    } finally {
  ...
  const handleAddDependency = async () => {
    ...
    try {
      ...
      showToast(error.response?.data?.error?.message || 'Failed to add dependency', 'error');
    } finally {
  ...
  const handleRemoveDependency = async (dependsOnTaskId) => {
    ...
    try {
      ...
      showToast(error.response?.data?.error?.message || 'Failed to remove dependency', 'error');
    }
  };
  ...
  const handlePostComment = async () => {
    ...
    try {
      ...
      showToast(error.response?.data?.error?.message || 'Failed to post comment', 'error');
    } finally {
  ...
  const handleUpdateComment = async (commentId) => {
    ...
    try {
      ...
      setTimeline((prev) =>
        prev.map((item) =>
          item.type === 'comment' && item.id === commentId ? updatedComment : item,
        ),
      );
      setEditingCommentId(null);
      setEditingCommentBody('');
      ...
      showToast(error.response?.data?.error?.message || 'Failed to update comment', 'error');
    } finally {
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      ...
      setTimeline((prev) =>
        prev.filter((item) => !(item.type === 'comment' && item.id === commentId)),
      );
      showToast('Comment deleted', 'success');
    } catch (error) {
      logger.error('Failed to delete comment', error);
      showToast(error.response?.data?.error?.message || 'Failed to delete comment', 'error');
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (internalTask) {
        ...
        // If user is currently viewing History tab, reload that data
        if (activeTab === 1) {
      ...
      if (error.response?.status === 409) {
        ...
        if (blockingTasks.length > 0) {
          const taskKeys = blockingTasks
            ...
            .filter(Boolean)
            .join(', ');
          showToast(`Task is blocked by: ${taskKeys}. Complete these tasks first.`, 'error');
        } else {
          showToast(
        ...
        showToast(error.response?.data?.error?.message || 'Failed to save task', 'error');
      }
    } finally {
  ...
  const handleArchive = async () => {
    if (
      internalTask &&
      window.confirm('Archive this task? It will be moved to the Archived section.')
    ) {
      try {
        ...
        showToast(error.response?.data?.error?.message || 'Failed to archive task', 'error');
      }
    }
  };
  ...
  const handleRestore = async () => {
    if (internalTask) {
      try {
        ...
        showToast(error.response?.data?.error?.message || 'Failed to restore task', 'error');
      }
    }
  };
                ...
                {mode === 'view' && internalTask && (
                  <>
                        ...
                        <div className="md:col-span-2">
                          {formData.summary && (
                            <div className="mb-4">
                              ...
                              <p className="text-sm text-dark-200 bg-dark-800 p-4 rounded-lg">
                                {formData.summary}
                              </p>
                            </div>
                          )}
                            ...
                            <div className="border-t border-dark-700 pt-4">
                              ...
                              <p className="text-dark-100">
                                {formData.preferred_model
                                  ? models.find((m) => m.id === formData.preferred_model)?.name ||
                                    formData.preferred_model
                                  : 'Default'}
                              </p>
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-6 mb-8">
                              ...
                              <div>
                                ...
                                >
                                  ...
                                  {availableTasks.map((task) => {
                                    ...
                                    const epicInfo = task.parent_task_number
                                      ? ` [Epic: TASK-${task.parent_task_number}]`
                                      : '';
                                    return (
                                      <option key={task.id} value={task.id}>
                                        {typeIcon} TASK-{task.task_number} - {task.title}
                                        {epicInfo}
                                      </option>
                                    );
                                  })}
                                      ...
                                      onClick={async () => {
                                        try {
                                          const task = await useTaskStore
                                            .getState()
                                            .fetchTaskById(dep.id);
                                          onClose();
                                          setTimeout(() => {
                                            window.dispatchEvent(
                                              new CustomEvent('openTask', { detail: task }),
                                            );
                                          }, 100);
                                        } catch (error) {
                      ...
                      <div className="bg-dark-800 p-4 rounded-lg">
                        <h3 className="text-sm font-medium text-dark-200 mb-3">Epic & Subtasks</h3>
                          ...
                          {internalTask.parent_task_id ? (
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
                                try {
                                  const parentTask = await useTaskStore
                                    .getState()
                                    .fetchTaskById(internalTask.parent_task_id);
                                  ...
                                  setTimeout(() => {
                                    ...
                                    window.dispatchEvent(
                                      new CustomEvent('openTask', { detail: parentTask }),
                                    );
                                  }, 100);
                                } catch (error) {
                              ...
                              {subtasks.map((sub) => (
                                <button
                                  ...
                                  onClick={async (e) => {
                                    e.stopPropagation();
                                    try {
                                      const subtask = await useTaskStore
                                        .getState()
                                        .fetchTaskById(sub.id);
                                      onClose();
                                      setTimeout(() => {
                                        window.dispatchEvent(
                                          new CustomEvent('openTask', { detail: subtask }),
                                        );
                                      }, 100);
                                    } catch (error) {
                    ...
                    {internalTask.status === TASK_STATUS.DONE && (
                      <div className="p-3 bg-blue-900/20 border border-blue-800 rounded-lg mb-6">
                        <p className="text-sm text-blue-300">
                          This task is marked as Done. It will be automatically archived after 7
                          days, or you can archive it manually.
                        </p>
                      </div>
                    )}
                          ...
                          <div className="space-y-4 min-h-[200px]">
                            {!internalTask || mode !== 'view' ? (
                              ...
                              <>
                                ...
                                {/* Comments only - activity events are shown in History tab */}
                                {loadingComments ? (
                                        ...
                                        return (
                                          ...
                                          >
                                            ...
                                            {isEditing ? (
                                              ...
                                              <MarkdownRenderer content={item.body} size="sm" />
                                            )}
                                          </div>
                                        );
                                      })}
                              ...
                              <>
                                  ...
                                  <div className="flex gap-3 mb-4">
                                    <div className="px-3 py-2 bg-dark-800/40 border border-dark-700/50 rounded-lg flex items-center gap-2">
                                      ...
                                      <span className="text-xs text-dark-400">
                                        Running: {subagentsMeta.running}
                                      </span>
                                    </div>
                                    <div className="px-3 py-2 bg-dark-800/40 border border-dark-700/50 rounded-lg flex items-center gap-2">
                                      <QueueListIcon className="w-4 h-4 text-yellow-400" />
                                      <span className="text-xs text-dark-400">
                                        Queued: {subagentsMeta.queued}
                                      </span>
                                    </div>
                                    <div className="px-3 py-2 bg-dark-800/40 border border-dark-700/50 rounded-lg flex items-center gap-2">
                                      <CheckCircleIcon className="w-4 h-4 text-blue-400" />
                                      <span className="text-xs text-dark-400">
                                        Completed: {subagentsMeta.completed}
                                      </span>
                                    </div>
                                ...
                                <div className="space-y-3">
                                  {subagents.map((attempt, idx) => {
                                    ...
                                    const config =
                                      statusConfig[attempt.status] || statusConfig.unknown;

                                    const startTime =
                                      attempt.startedAt || attempt.queuedAt || attempt.completedAt;
                                    ...
                                    return (
                                      ...
                                      >
                                        <div className="flex items-start justify-between mb-3">
                                          <div className="flex items-center gap-3">
                                            <span
                                              className={`px-2 py-1 ${config.bgColor} ${config.color} text-xs font-medium rounded`}
                                            >
                                              {config.label}
                                            </span>
                                            {attempt.model && (
                      ...
                      <div className="md:col-span-2 space-y-4">
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={handleChange}
                            className="input-field"
                            placeholder="Full task description (supports Markdown)"
                          />
                        </div>
                      </div>
