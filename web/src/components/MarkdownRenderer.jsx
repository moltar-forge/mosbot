...
const MarkdownRenderer = ({
  ...
  const components = useMemo(
    () => ({
      ...
      p: ({ node: _node, children, ...props }) => {
        const text = getTextFromChildren(children)?.trim() ?? '';
        const pEl = (
          <p className={`${textSize} text-dark-200 mb-3 last:mb-0 leading-relaxed`} {...props}>
            {children}
          </p>
        );
        if (!text || text.length > 50000) return pEl;
        return <SmartContentBlock content={text} fallback={pEl} />;
      },
      ...
      li: ({ node: _node, ...props }) => {
        ...
        return <li className={`${textSize} text-dark-200`} {...props} />;
      },
      ...
      code: ({ node, ...props }) => {
        ...
        const content =
          typeof props.children === 'string' ? props.children : String(props.children?.[0] ?? '');
      ...
      // Horizontal rule
      hr: ({ node: _node, ...props }) => (
        <hr className={`border-dark-700 ${isExtraSmall ? 'my-2' : 'my-3'}`} {...props} />
      ),
      ...
      em: ({ node: _node, ...props }) => <em className="italic text-dark-200" {...props} />,
      ...
      thead: ({ node: _node, ...props }) => <thead className="bg-dark-900" {...props} />,
      tbody: ({ node: _node, ...props }) => <tbody {...props} />,
      tr: ({ node: _node, ...props }) => <tr className="border-b border-dark-700" {...props} />,
      th: ({ node: _node, ...props }) => (
        <th
  ...
  return (
    <div className={`prose prose-invert ${proseSize} max-w-none text-dark-200 ${className}`}>
