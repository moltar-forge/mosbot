    ...
    <div className="p-5 bg-dark-800 border border-dark-700 rounded-lg hover:border-dark-600 transition-colors">
      ...
      {prompt && (
        <div className="mt-4 ml-8 p-3 bg-dark-900/50 border border-dark-700/50 rounded">
          ...
          <p className="text-sm text-dark-300 line-clamp-2 leading-relaxed">{prompt}</p>
        </div>
      )}
          ...
          {agentId && (
            <>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-dark-200 font-medium">{job.agentName || agentId}</span>
              </div>
            </>
          )}
          {job.sessionTarget && (
            <>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-dark-200 font-mono">{job.sessionTarget}</span>
              </div>
            </>
          )}
          {model && (
            <>
              {(deliveryMode !== 'none' || agentId || job.sessionTarget) && (
    ...
    <div className="bg-dark-900 border border-dark-800 rounded-lg p-6">
      {isLoading ? (
        <div className="flex items-center justify-center py-8">
          ...
          <span className="text-sm text-dark-500 ml-3">Loading cron jobs...</span>
        </div>
      ) : jobs.length === 0 ? (
        <p className="text-sm text-dark-500 text-center py-8">No cron jobs configured</p>
      ) : (
