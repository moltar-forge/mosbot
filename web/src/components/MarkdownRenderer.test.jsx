...
- Item 3
  ...
  it('applies custom className', () => {
    const { container } = render(
      <MarkdownRenderer content="Test content" size="sm" className="custom-class" />,
    );
  ...
  it('renders with xs size variant', () => {
    const { container } = render(<MarkdownRenderer content="Test content" size="xs" />);
    const wrapper = container.firstChild;
...
- **Base URL**: \`example.com/api\``;
  ...
  it('handles markdown with inline code in bullet points like AGENTS.md', () => {
    ...
    const content =
      '- Before creating any new file, verify its placement against `docs/WORKSPACE_CONVENTIONS.md`.';
    const { container } = renderWithRouter(<MarkdownRenderer content={content} size="sm" />);
...
This is the actual content that should be displayed.`;
    ...
    expect(
      screen.getByText('This is the actual content that should be displayed.'),
    ).toBeInTheDocument();
  });
