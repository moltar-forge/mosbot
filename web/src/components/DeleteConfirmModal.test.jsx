...
describe('DeleteConfirmModal', () => {
  ...
  it('does not render when isOpen is false', () => {
    ...
    render(<DeleteConfirmModal isOpen={false} onClose={mockOnClose} file={file} />);
  ...
  it('renders modal when open with file', () => {
    const file = { name: 'test.txt', path: '/test.txt', type: 'file' };
    render(<DeleteConfirmModal isOpen={true} onClose={mockOnClose} file={file} />);
  ...
  it('shows folder-specific message for directories', () => {
    ...
    expect(
      screen.getByText(/Warning: This will delete the folder and all its contents recursively/),
    ).toBeInTheDocument();
  });
  ...
  it('calls onClose when close button is clicked', async () => {
    ...
    const closeButton =
      container.querySelector('button[disabled]') || container.querySelector('button:has(svg)');
  ...
  it('deletes file and shows success toast', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockShowToast).toHaveBeenCalledWith('File "test.txt" deleted successfully', 'success');
      expect(mockOnClose).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it('deletes folder and shows success toast', async () => {
    ...
    await waitFor(() => {
      ...
      expect(mockFetchListing).toHaveBeenCalled();
      expect(mockShowToast).toHaveBeenCalledWith('Folder "folder" deleted successfully', 'success');
    });
  });
